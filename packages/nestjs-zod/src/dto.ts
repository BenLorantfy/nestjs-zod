...
import { $ZodType, JSONSchema, toJSONSchema } from 'zod/v4/core'
...
function openApiMetadataFactory({
  ...
  const { $defs, $schema, ...generatedJsonSchema } = generateJsonSchema(
    schema,
    io
  )
...
function generateJsonSchema(
  ...
  // @ts-expect-error
  const newSchema = walkJsonSchema(
    generatedJsonSchema,
    (schema) => {
