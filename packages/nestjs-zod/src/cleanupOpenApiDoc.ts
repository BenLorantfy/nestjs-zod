...
export function cleanupOpenApiDoc(
  ...
  for (const [oldSchemaName, oldOpenapiSchema] of Object.entries(
      ...
      if (DEFS_KEY in propertySchema) {
        ...
        if (!addedDefs) {
          ...
          for (const [defSchemaId, defSchema] of Object.entries(defs)) {
            ...
            if (
              schemas[newDefSchemaKey] &&
              !isDeepStrictEqual(schemas[newDefSchemaKey], fixedDef)
            ) {
              throw new Error(
                `[cleanupOpenApiDoc] Found multiple schemas with name \`${newDefSchemaKey}\`.  Please review your schemas to ensure that you are not using the same schema name for different schemas`
              )
            }
    ...
    if (
      ...
      !isDeepStrictEqual(schemas[newSchemaName], newOpenapiSchema)
    ) {
      throw new Error(
  ...
  for (const { get, patch, post, delete: del, put, head } of Object.values(
    ...
    for (const methodObject of Object.values({
      ...
      for (const statusCodeObject of Object.values(
        ...
        for (const responseBodyObject of Object.values(content)) {
          if (
            responseBodyObject.schema &&
            '$ref' in responseBodyObject.schema
          ) {
            const oldSchemaName = getSchemaNameFromRef(
      ...
      for (const parameter of methodObject?.parameters || []) {
          ...
          for (const [defSchemaId, defSchema] of Object.entries(defs)) {
            ...
            try {
              ...
              if (
                ...
                error.message.startsWith('[fixAllRefs]')
              ) {
                throw new Error(
            ...
            if (
              schemas[defSchemaId] &&
              !isDeepStrictEqual(schemas[defSchemaId], fixedDef)
            ) {
              throw new Error(
                `[cleanupOpenApiDoc] Found multiple schemas with name \`${defSchemaId}\`.  Please review your schemas to ensure that you are not using the same schema name for different schemas`
              )
            }
