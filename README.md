...
```
This command runs a codemod that adds the validation pipe, serialization interceptor, http exception filter, and swagger cleanup function. If you opt into swagger/OpenAPI generation, it will also offer to scaffold [Specmatic](https://specmatic.io/) contract testing (an `openapi.json` generator script, `generate:openapi`/`test:contract` scripts, and a `specmatic-examples/` fixtures directory) so your API's documented responses can be verified against what your app actually returns
...
| id      | _Omitted_                             | `{ id: 'Author', type: 'object', ... }`            |
