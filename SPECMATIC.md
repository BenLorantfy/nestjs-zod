...
Everything above only protects this repo's own reference app. `nestjs-zod-cli` (`packages/cli`) — the codemod tool behind the README's "Automatic Setup" (`npx nestjs-zod-cli /path/to/nestjs/project`) — does not scaffold Specmatic into consumer projects; OpenAPI/spec generation is left entirely to the consumer's own app code (the same `SwaggerModule.createDocument` + `cleanupOpenApiDoc` call documented in section 2), following the pattern documented in sections 2 and 6. A consumer who wants the same contract-testing coverage would need to produce their own `openapi.json` (whether hand-maintained, like this repo's now is, or generated by a script of their own), add a `test:contract` npm script, and populate a `specmatic-examples/` fixtures directory by hand, using this repo's `packages/example` as a reference.
