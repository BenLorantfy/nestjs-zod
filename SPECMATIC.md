...
Everything above only protects this repo's own reference app. `nestjs-zod-cli` (`packages/cli`) — the codemod tool behind the README's "Automatic Setup" (`npx nestjs-zod-cli /path/to/nestjs/project`) — creates a `specmatic.yaml` and a `test:contract` script for consumer projects when swagger/OpenAPI generation is set up, but nothing more; OpenAPI/spec generation itself is still left entirely to the consumer's own app code (the same `SwaggerModule.createDocument` + `cleanupOpenApiDoc` call documented in section 2), following the pattern documented in section 2.

**This is deliberately a different, simpler setup than `packages/example`'s own, not an incomplete copy of it.** The CLI's `specmatic.yaml.template` declares a `filesystem` source (`{ filesystem: { directory: "." } }`) pointing at a local `openapi.json` — because the CLI's swagger codemod (`swaggerTransform.js`) wires up `SwaggerModule.setup('api', app, cleanupOpenApiDoc(openApiDoc))` without the `jsonDocumentUrl` option that `packages/example`'s `main.ts` adds by hand, a CLI-scaffolded app doesn't serve its spec as JSON anywhere by default. A consumer who runs the CLI needs to either write their own `openapi.json` to disk (e.g. with a small script calling `NestFactory.create` + `SwaggerModule.createDocument` + `cleanupOpenApiDoc`, similar to what `packages/example` used before section 9's live-doc switch), or add `jsonDocumentUrl` to their own `main.ts` and adjust the generated `specmatic.yaml` to a `web` source themselves, following `packages/example`'s current config as a reference. Either way, they'll also want to populate a `specmatic-examples/` fixtures directory by hand and consider turning on `schemaResiliencyTests` (already on by default in the scaffolded config) — see section 7.3 for why that setting is worth keeping.
